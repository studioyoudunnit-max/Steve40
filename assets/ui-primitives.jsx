// UI Primitives — logo, buttons, layout shells, confetti, sparks
const { useEffect, useState, useRef, useMemo, useCallback } = React;

// ═══════════════════════════════════════════
// LOGO
// ═══════════════════════════════════════════
function Logo({ size='md', glow=true }) {
  const sizes = {
...
// Compact inline logo for toolbars
function MiniLogo({ subtitle }) {
  return (
...
// BUTTONS
// ═══════════════════════════════════════════
function Btn({ children, kind='primary', size='md', full=false, onClick, disabled=false, style={}, icon }) {
  const sizes = {
  ...
  return (
    ...
    >
      {icon && <svg width="18" height="18"><use href={`#${icon}`}/></svg>}
      {children}
    </button>
  );
}
...
// ═══════════════════════════════════════════
// STAGE WRAPPER — full-bleed game screen shell
// ═══════════════════════════════════════════
function Stage({ children, pad=true, style={} }) {
  return (
    <div style={{
...
// ═══════════════════════════════════════════
// HEADER BAR (persistent across game screens)
// ═══════════════════════════════════════════
function GameHeader({ title, subtitle, onBack, right, accent }) {
  return (
    <div style={{
...
// ═══════════════════════════════════════════
// TIMER (progress ring + numeric)
// ═══════════════════════════════════════════
function TimerBar({ value, max, accent='var(--accent-2)', big=false }) {
...
// Floating +points
function PointsPop({ pts, shown, onDone }) {
  useEffect(() => {
    if (shown) {
...
// ═══════════════════════════════════════════
// BEAT PULSE (sound-ready visual cue)
// ═══════════════════════════════════════════
function BeatPulse({ color='var(--accent-1)', intensity=1 }) {
  return (
    <div style={{
...
// ═══════════════════════════════════════════
// QR CODE PLACEHOLDER (SVG grid, looks like QR)
// ═══════════════════════════════════════════
function QRPlaceholder({ size=170 }) {
  const cells = useMemo(() => {
    const N = 25;
    ...
    // 3 finder patterns (7x7) in corners
...
Object.assign(window, {
  Logo, MiniLogo, Btn, Card, Stage, GameHeader, TimerBar,
  Confetti, PointsPop, TeamDot, TeamChip, BeatPulse, Sparks, QRPlaceholder,
});
