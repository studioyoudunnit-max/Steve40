...
export function TriviaScreen({ onBack, onNextGame }) {
...
export function TwinkLesbianScreen({ onBack, onNextGame }) {
  ...
  const [tally, setTally] = useState({ twink: 0, lesbian: 0 });
  useEffect(() => {
    if (revealed) return;
...
export function LipsyncScreen({ onBack, onNextGame }) {
  ...
  const goJudge = () => {
    ...
    const seedVotes = () => {
      const next = {};
      TEAMS.slice(0, 6).forEach(t => { next[t.id] = Math.floor(Math.random() * 18); });
      setVotes(next);
    };
    seedVotes();
    const id = setInterval(() => {
...
export function SongpopScreen({ onBack, onNextGame }) {
